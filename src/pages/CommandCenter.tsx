...
export function CommandCenter() {
  const { snapshots, competitors, projects, report, lastRefreshedAt, refreshIntervalMinutes } = useDataStore();
  const latest = snapshots[snapshots.length - 1];
  const prev = snapshots[snapshots.length - 2] ?? latest;
  const changePct = (a: number, b: number) => (((a - b) / Math.max(b, 1)) * 100).toFixed(1);

  const metrics = [
    { label: 'Total Views', value: latest.views.toLocaleString(), trend: `${changePct(latest.views, prev.views)}%`, isPositive: latest.views >= prev.views },
    { label: 'Subscribers', value: `${latest.subs.toLocaleString()}`, trend: `${changePct(latest.subs, prev.subs)}%`, isPositive: latest.subs >= prev.subs },
    { label: 'Search Volume', value: `${report.searchVolume}/100`, trend: `${report.searchInterestPct}%`, isPositive: true },
    { label: 'Est. Revenue', value: `$${latest.revenue.toLocaleString()}`, trend: `${changePct(latest.revenue, prev.revenue)}%`, isPositive: latest.revenue >= prev.revenue },
  ];

  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-black text-text-1 tracking-tight">Command Center</h1>
      <p className="text-sm text-text-3">Data refreshes automatically every {refreshIntervalMinutes} minute(s). Last sync: {new Date(lastRefreshedAt).toLocaleTimeString()}.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-bg-surface border border-border rounded-xl shadow-sm overflow-hidden flex flex-col h-[350px]">
            ...
            <div className="p-4 text-sm text-text-2">Latest AI topic: <span className="font-bold text-text-1">{report.topic}</span><div className="mt-3 flex flex-wrap gap-2">{report.emergingTopics.map((t) => <span key={t} className="text-xs bg-primary-muted text-primary px-2 py-1 rounded-full">{t}</span>)}</div></div>
          </div>
          ...
          <div className="bg-bg-surface border border-border rounded-xl shadow-sm p-6">
            ...
            <table className="w-full text-left border-collapse"><thead><tr className="border-b border-border text-xs uppercase tracking-wider text-text-3"><th className="pb-3">Competitor</th><th className="pb-3">Subscribers</th><th className="pb-3">Growth</th></tr></thead><tbody>
        ...
        <div className="space-y-6">
          <div className="bg-bg-surface border border-border rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">{projects.slice(0, 4).map((p) => <div key={p.id} className="border border-border rounded-lg p-3"><p className="font-bold text-sm">{p.title}</p><p className="text-xs text-text-3 mt-1">{p.targetDate} • {p.status}</p></div>)}</div>
          </div>
